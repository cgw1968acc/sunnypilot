...
from openpilot.selfdrive.controls.lib.latcontrol_torque import (apply_curve_outward_bias, CURVE_OUTWARD_FRAC,
                                                                CURVE_OUTWARD_DEADZONE, CURVE_OUTWARD_V_HI, CURVE_OUTWARD_V_LO)
...
class TestCurveOutwardBias(unittest.TestCase):
  def test_straights_and_small_corrections_untouched(self):
    for k in (0.0, 0.001, CURVE_OUTWARD_DEADZONE, -CURVE_OUTWARD_DEADZONE, 0.0025):
      self.assertEqual(apply_curve_outward_bias(k, CURVE_OUTWARD_V_HI), k)

  def test_curves_relaxed_outward_and_sign_preserved(self):
    for k in (0.0067, 0.012, -0.0067, -0.012):
      out = apply_curve_outward_bias(k, CURVE_OUTWARD_V_HI)
  ...
  def test_gentle_high_speed_curves_are_biased(self):
    ...
    self.assertLess(abs(apply_curve_outward_bias(k, CURVE_OUTWARD_V_HI)), abs(k))

  def test_reduction_is_a_uniform_fraction_above_the_deadzone(self):
    for k in (0.006, 0.010, 0.02):
      self.assertAlmostEqual(apply_curve_outward_bias(k, CURVE_OUTWARD_V_HI), k * (1.0 - CURVE_OUTWARD_FRAC), places=9)
  ...
  def test_no_bias_at_low_speed(self):
    ...
    for k in (0.006, 0.02, -0.02):
      self.assertEqual(apply_curve_outward_bias(k, CURVE_OUTWARD_V_LO), k)
  ...
  def test_bias_is_bounded(self):
    ...
    for k in (0.05, -0.05):
      self.assertLessEqual(abs(k - apply_curve_outward_bias(k, CURVE_OUTWARD_V_HI)), CURVE_OUTWARD_FRAC * abs(k) + 1e-9)
