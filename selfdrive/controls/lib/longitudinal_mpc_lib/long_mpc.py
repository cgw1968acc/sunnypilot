...
def get_stopped_equivalence_factor_krkeegen(v_lead, v_ego):
  ...
  if np.any(mask):
    ...
    scaling_factor = np.interp(v_ego, [0, 3, 8, 16], [2.4, 2.2, 2.2, 1.8])  # Increased low-speed response
    ...
    ego_scaling = np.interp(v_ego, [0, 1, 10, 20], [1.0, 1.0, 0.85, 0.85])  # Slightly stronger response at low speed
