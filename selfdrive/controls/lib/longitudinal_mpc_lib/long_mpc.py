...
class LongitudinalMpc:
  ...
  def reset(self):
    ...
    self.distance_to_stop_target_m: float = -1.0
    self._stop_target_latch_s: float = 0.0
    self.set_weights()
  ...
  def update(self, radarstate, v_cruise, personality=log.LongitudinalPersonality.standard, a_cruise_min=None, t_follow=None):
    ...
    # Track distance to stopped lead target for longcontrol stop approach logic
    lead_0_stop = get_distance_to_stopped_lead_target(lead_xv_0[:,1], lead_xv_0[:,0]) if radarstate.leadOne.status else 0.0
    lead_1_stop = get_distance_to_stopped_lead_target(lead_xv_1[:,1], lead_xv_1[:,0]) if radarstate.leadTwo.status else 0.0
    self.distance_to_stop_target_m, self._stop_target_latch_s = update_distance_to_stop_target_with_latch(
      self.distance_to_stop_target_m, self._stop_target_latch_s, DT_MDL, (lead_0_stop, lead_1_stop)
    )
