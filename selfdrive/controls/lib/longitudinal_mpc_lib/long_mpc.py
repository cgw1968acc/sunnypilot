...
def get_stopped_equivalence_factor_krkeegen(v_lead, v_ego):
  ...
  if np.any(mask):
    ...
    scaling_factor = np.interp(v_ego, [0, 4, 8], [2.4, 2.4, 2.0])  # Try tweaking the [2.0, 1.2] range
    ...
    ego_scaling = np.interp(v_ego, [0, 10, 20], [1.13, 1.07, 0.90])   # Try adjusting [1.0, 0.3] for different response
