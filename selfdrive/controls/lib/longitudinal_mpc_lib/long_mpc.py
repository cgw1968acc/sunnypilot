...
def get_stopped_equivalence_factor_krkeegen(v_lead, v_ego):
  ...
  if np.any(mask):
    ...
    scaling_factor = np.interp(v_ego, [0,  2,  8, 16], [2.4, 2.0, 2.0, 1.6])  # Try tweaking the [2.0, 1.2] range
    ...
    ego_scaling = np.interp(v_ego, [0, 1., 10, 20], [1.0,  1.15, 1.10, 0.90])   # Try adjusting [1.0, 0.3] for different response
