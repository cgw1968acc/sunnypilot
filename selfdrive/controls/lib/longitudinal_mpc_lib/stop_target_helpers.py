...
def update_distance_to_stop_target_with_latch(
  ...
  if valid:
    best = max(valid)
    # Hold the maximum (earliest/furthest) capture. Only update upward while
    # the lead is present so the target is not dragged down during approach.
    held = max(best, last) if last > 0.0 else best
    return held, STOP_TARGET_LATCH_S
