...
from openpilot.sunnypilot.selfdrive.controls.lib.longcontrol_sp import LongControlSP
...
class ControlsExt(ModelStateBase):
  ...
  def initialize_longitudinal_control(self, loc):
    return LongControlSP(self.CP, self.CP_SP)
  ...
  def long_update(self, long_active, CS, a_target, should_stop, accel_limits):
    lead_one = self.sm['radarState'].leadOne
    long_plan_sp = self.sm['longitudinalPlanSP']
    distance_to_stop_target_m = float(long_plan_sp.distanceToStopTarget) if long_plan_sp.distanceToStopTarget > 0.0 else None
    return self.LoC.update(
      long_active, CS, a_target, should_stop, accel_limits,
      distance_to_stop_target_m=distance_to_stop_target_m,
      lead_status=bool(lead_one.status),
      lead_v=float(lead_one.vLead),
