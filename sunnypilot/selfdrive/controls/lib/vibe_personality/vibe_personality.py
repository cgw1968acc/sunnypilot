...
MAX_ACCEL_PROFILES = {
    AccelPersonality.eco:       [2.0,  1.98,  1.90, 1.10, .500, .292, .13, .09],   # eco
    AccelPersonality.normal:    [2.0,  1.99,  1.95, 1.22, .635, .33, .18, .14],   # normal
...
MIN_ACCEL_PROFILES = {
    LongPersonality.relaxed:    [-.0008, -.0008, -.1,  -.4,  -.8,  -1.1],  # gentler braking
    LongPersonality.standard:   [-.0012, -.0012, -.2,  -.5,  -.9,  -1.2],  # normal braking
    LongPersonality.aggressive: [-.0020, -.0020, -.3,  -.6,  -1.0, -1.2],  # more aggressive braking
}
MIN_ACCEL_BREAKPOINTS =         [0.,   1.5,    3.,   11.,   16.,  50.]

# Follow distance profiles mapped to LongPersonality (relaxed/standard/aggressive)
FOLLOW_PROFILES = {
    LongPersonality.relaxed:    [1.40, 1.45, 1.45, 1.65, 1.65, 1.80, 1.80],  # more spread out
    LongPersonality.standard:   [1.20, 1.25, 1.25, 1.45, 1.45, 1.50, 1.50],  # balanced
    LongPersonality.aggressive: [1.00, 1.04, 1.04, 1.17, 1.17, 1.25, 1.27],  # tighter
}
