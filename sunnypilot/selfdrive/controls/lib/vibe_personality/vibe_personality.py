...
MAX_ACCEL_PROFILES = {
    ...
    AccelPersonality.normal:    [2.00, 2.0,  1.95, 1.40, .600, .299, .18, .13],   # normal
...
MIN_ACCEL_PROFILES = {
    LongPersonality.relaxed:    [-.006, -1.1, -1.1, -1.1],  # gentler braking
    LongPersonality.standard:   [-.008, -1.2, -1.2, -1.2],  # normal braking
    LongPersonality.aggressive: [-.010, -1.2, -1.2, -1.2],  # more aggressive braking
}
...
def get_dynamic_personality(v_ego, personality):
    ...
    scale_factor = np.interp(v_ego, [0, 22, 36], [1.0, 1.0, 1.9])
