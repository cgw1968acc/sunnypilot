...
MIN_ACCEL_PROFILES = {
    LongPersonality.relaxed:    [-1.000, -1.1],  # gentler braking
    LongPersonality.standard:   [-1.100, -1.2],  # normal braking
    LongPersonality.aggressive: [-1.200, -1.2],  # more aggressive braking
}
MIN_ACCEL_BREAKPOINTS =         [0.,    50.]

# Follow distance profiles mapped to LongPersonality (relaxed/standard/aggressive)
FOLLOW_PROFILES = {
    LongPersonality.relaxed:    [1.75, 1.75, 1.65, 1.80, 1.80],  # more spread out
    LongPersonality.standard:   [1.45, 1.45, 1.40, 1.50, 1.50],  # balanced
    LongPersonality.aggressive: [1.25, 1.25, 1.25, 1.25, 1.30],  # tighter
}
